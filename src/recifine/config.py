...
def apply_layered_yaml_to_args(
    ...
    *,
    ...
    if config_type == "modeling":
        ...
        # Base keys you want to expose to the rest of the code
        variant_driven_keys = (
            "data_dir",
            "output_dir",
            "labels",
        ...
        for path_key in ("data_dir", "output_dir", "labels", "cache_dir", "config_name", "tokenizer_name"):
    ...
    # condition that works with dataset preprocessing
        ...
        for path_key in ("original_dataset", "data_dir_ka", "dataset_to_evaluate_ka", "data_dir_trad", "dataset_to_evaluate_trad"):
            if hasattr(args, path_key):
                val = getattr(args, path_key)
                if isinstance(val, str) and val.strip():
                    setattr(args, path_key, str(resolve_path_maybe_relative(val, repo_root)))

    # condition that works with prediction wrapper
        ...
        # resolve path if provided by user  # check if it works if user provides hugging face path
            ...
            for path_key in ("model_name_or_path",):
                if hasattr(args, path_key):
                    val = getattr(args, path_key)
                    if isinstance(val, str) and val.strip():
                        setattr(args, path_key, str(resolve_path_maybe_relative(val, repo_root)))
