...
def preprocess_gk_ka(args, seed: int = 42, overwrite: bool = False) -> None:
    ...
    test_folders = getattr(args, "dataset_to_evaluate_ka", None) or []  # optional

    if not os.path.isfile(input_path):
        raise FileNotFoundError(f"GK input not found: {input_path}")
    ...
    logger.info("Reading GK: %s", input_path)
...
def preprocess_gk_trad(args, seed: int = 42, overwrite: bool = False) -> None:
    in_root = getattr(args, "data_dir_ka", None)
    out_root = getattr(args, "data_dir_trad", None)
    if not in_root or not out_root:
        raise ValueError("preprocess_gk_trad requires args.data_dir_ka and args.data_dir_trad")
