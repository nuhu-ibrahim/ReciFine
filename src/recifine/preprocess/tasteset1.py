...
def _find_line_by_span(recipe_text: str, start: int, end: int) -> tuple[int | None, str | None]:
...
def _split_sentences_with_entity_balance(
    ...
    for entity, entity_sents in entity_to_sentences.items():
        ...
        for sent in entity_sents:
            ...
            if (
                (len(train_local) < train_size and sentence_hashable not in val_sentences and sentence_hashable not in test_sentences)
                or (sentence_hashable in train_sentences)
            ):
                splits["train"].append(sent)
                train_sentences.add(sentence_hashable)
                train_local.append(sentence_hashable)

            elif (
                (len(val_local) < val_size and sentence_hashable not in train_sentences and sentence_hashable not in test_sentences)
                or (sentence_hashable in val_sentences)
            ):
                splits["val"].append(sent)
                val_sentences.add(sentence_hashable)
                val_local.append(sentence_hashable)

            elif (
                (len(test_local) < test_size and sentence_hashable not in train_sentences and sentence_hashable not in val_sentences)
                or (sentence_hashable in test_sentences)
            ):
                splits["test"].append(sent)
                test_sentences.add(sentence_hashable)
                ...
                logger.debug(
                    ...
                    entity,
                    (len(val_local) < val_size and sentence_hashable not in train_sentences and sentence_hashable not in test_sentences),
                    (sentence_hashable in val_sentences),
                    (len(test_local) < test_size and sentence_hashable not in train_sentences and sentence_hashable not in val_sentences),
                    (sentence_hashable in test_sentences),
                    sent,
                )
        ...
        for answer in ann.get("answer", []):
            ...
            for i in range(len(tokens) - len(answer_tokens) + 1):
                if tokens[i : i + len(answer_tokens)] == answer_tokens:
                    if tags[i] == "O":  # avoid overwrite (preserved)
                        tags[i] = f"B-{entity}"
...
def preprocess_tasteset1_ka(args, seed: int = 42, overwrite: bool = False) -> None:
    ...
    if isinstance(input_csv, (list, tuple)):
        if len(input_csv) != 1:
            raise ValueError(f"TasteSet1 expects a single CSV path, got {len(input_csv)} paths: {input_csv}")
    ...
    with open(input_csv, "r", encoding="utf-8") as csvfile:
        ...
        for row in reader:
            ...
            for entity in entities:
                ...
                qid, recipe_line = _find_line_by_span(recipe_text, start, end)
                    ...
                    rows.append(
                        {
                            ...
                            "definition": entity_group.get("definition"),
...
def preprocess_tasteset1_trad(args, seed: int = 42, overwrite: bool = False) -> None:
    in_root = getattr(args, "data_dir_ka", None)
    out_root = getattr(args, "data_dir_trad", None)
    if not in_root or not out_root:
        raise ValueError("preprocess_tasteset1_trad requires args.data_dir_ka and args.data_dir_trad")
